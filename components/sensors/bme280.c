/**
 * @file    bme280.c
 * @brief   Implementation of functions for interacting with the BME280 sensor.
 *
 * @date    June, 2024
 */
...
esp_err_t bmx280_readout(bmx280_t *bmx280, int32_t *temperature, uint32_t *humidity)
{
    if (bmx280 == NULL) return ESP_ERR_INVALID_ARG;
    if (!bmx280_validate(bmx280)) return ESP_ERR_INVALID_STATE;
 ...
 *
 * This function configures the I2C peripheral and installs the driver for
 * communication with the BME280 sensor.
 */
...
/**
 * @brief   Task function to handle communication with the BME280 sensor.
 *
 * This task initializes the BME280 driver, configures it, and continuously reads
 * temperature and humidity data from the sensor, logging the values periodically.
 *
 * @param   arg     Unused argument for FreeRTOS task signature.
 */
void i2c_bme280_task(void *arg)
{
    ...
    while (1)
    {
        ...
        ESP_ERROR_CHECK(bmx280_readoutFloat(bmx280, &sensorData.temperature, &sensorData.humidity));

        ESP_LOGI("BME280_TASK", "Read Values: temp = %.4f, hum = %.4f", sensorData.temperature, sensorData.humidity);
